...
public class QDataLoader extends DefaultHandler {
  ...
  public void startElement(String uri, String localName,
    ...
    currentElement = "`" + qName + "' element";

    try {
      switch (qName) {
        case "channel"            -> handleChannel(attributes);
        case "patch"              -> handlePatch(attributes);
        case "patch-alias"        -> { if (!ignorePatchAliases) handlePatchAlias(attributes); }
        case "cue-stream"         -> handleCueStream(attributes);
        case "cue"                -> handleCue(attributes);
        case "program-change"     -> handleProgramChange(attributes);
        case "note-window-change" -> handleNoteWindowChange(attributes);
        case "advance"            -> handleAdvance(attributes);
        case "trigger"            -> handleTrigger(attributes);
        case "events"             -> buildingEvents = true;
        case "note-on"            -> handleNoteOn(attributes);
        case "note-off"           -> handleNoteOff(attributes);
        case "control-change"     -> handleControlChange(attributes);
        case "map-events"         -> curMapper = new EventMapper();
      }
  ...
  private void handleChannel(Attributes attributes) {
    auxValue[0] = attributes.getValue("num");
    auxValue[1] = attributes.getValue("device-type");
  }

  private void handlePatch(Attributes attributes) {
    ...
    if (attributes.getValue("volume") != null) {
      ...
      patch.setVolume( parseIntOrPercent( attributes.getValue("volume"), 127 ) );
    }
  }

  private void handlePatchAlias(Attributes attributes) {
  ...
  private void handleCueStream(Attributes attributes) {
    qstream = new QStream();
  ...
  private void handleProgramChange(Attributes attributes) {
  ...
  private void handleNoteWindowChange(Attributes attributes) {
  ...
  private void handleAdvance(Attributes attributes) {
  ...
  private void handleTrigger(Attributes attributes) {
    currentAttribute = "reverse";
    String rev = attributes.getValue("reverse");
    reverseTrigger = rev != null && !rev.equals("false");
  ...
  private void handleNoteOn(Attributes attributes) {
    currentAttribute = "channel";
    int ch = Integer.parseInt(attributes.getValue("channel")) - 1;
  ...
  private void handleNoteOff(Attributes attributes) {
    currentAttribute = "channel";
    int ch = Integer.parseInt(attributes.getValue("channel")) - 1;
    currentAttribute = "note";
  ...
  private void handleControlChange(Attributes attributes) {
    currentAttribute = "channel";
    int ch = Integer.parseInt(attributes.getValue("channel")) - 1;
  ...
  public void endElement(String uri, String localName, String qName) {
    switch (qName) {
      ...
      case "channel"    -> endChannel();
      case "patch"      -> endPatch();
      case "patch-alias"-> { if (!ignorePatchAliases) endPatch(); }
      case "trigger"    -> endTrigger();
      case "map-from"   -> curMapper.setFromTemplate(curTemplate);
      case "map-to"     -> curMapper.addToTemplate(curTemplate);
      case "map-events" -> { eventMaps.add(curMapper); curMapper = null; }
      case "events"     -> buildingEvents = false;
      case "sysex"      -> endSysex();
      case "cue"        -> endCue();
      case "global"     -> endGlobal();
      case "cue-stream" -> { qdata.addCueStream(qstream); qstream = null; }
    }
  }
