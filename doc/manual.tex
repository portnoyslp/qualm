...
\item[volume] When loading this patch, tell the synthesizer to set the
  ...
  number from 0-127, or can be written as a percentage using a {\tt
    ...
    uses standard MIDI controls for changing a channel's volume, it
    may interact poorly with volume pedals or sliders which are meant
    to perform the same effect.  If you're using devices such as these
    in your synthesizer setup, it may be best to avoid using the {\tt
      volume} attribute.}
\end{description}
...
scenarios where a patch should be played noticably louder or softer than
